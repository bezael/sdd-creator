...
- **Install layout for non-Claude agents:** Codex, Cursor, Gemini CLI, Aider and Continue installs now copy each skill into `.agents/<skill>/` instead of copying the SDD `AGENTS.md` and `templates/` over the project root. The root `AGENTS.md` is reserved for the lane generated by `dominicode-harness-init`; Cursor (`.cursor/rules/dominicode.mdc`) and Gemini (`GEMINI.md` with `@AGENTS.md`) point to it. Claude Code per-project and Claude.ai installs include both skills.
  - **Upgrading from 1.7.x (non-Claude agents):** re-running the copy commands leaves the old files. Delete or move into `.agents/dominicode-sdd-creator/` the root `AGENTS.md` copied from the SDD skill, `GEMINI.md`, `.cursor/rules/dominicode-sdd-creator.mdc`, `./templates` and `./references` before running `dominicode-harness-init`. harness-init now detects that old root file and proposes replacing it with the lane instead of merging into it.
- **Light contract location:** `dominicode-harness-init` always generates `.dominicode/spec.template.md`, including in SDD repos, and filled copies live at `.dominicode/specs/<issue-number>-<slug>.md`. The template is never filled in place.
- Repository: `.gitattributes` enforces LF line endings; `specs/INDEX.md` records the CBRM shared decisions.
